...
| **Ruleset** | The PCE object the operator compiles segmentation policy into. Scoped to a namespace's identity labels (`app`+`env` by default; configurable via `policyScopeLabels`, `loc` intentionally excluded). |
