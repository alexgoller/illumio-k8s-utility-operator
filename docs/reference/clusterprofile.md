...
| `onboarding.nodePairingProfile.enforcementMode` | string | no | Enforcement mode for a newly created Pairing Profile. One of `idle`, `visibility_only`, `selective`, `full`. Defaults to `idle`. |
...
| `policyScopeLabels` | []string | no | Illumio label keys that form the per-namespace ruleset scope. A scope may be any number of labels; defaults to `[app, env]` when empty. Assigned labels not in this set (e.g. `loc`) stay on workloads for visibility but are excluded from the scope. `loc` is a poor scope choice. |
...
| `systemNamespaces.enforcementMode` | string | no | Enforcement mode for system namespaces. One of `idle`, `visibility_only`, `selective`, `full`. Defaults to `idle`. |
...
| `namespaceRules[].enforcementMode` | string | no | Enforcement mode for matching namespaces. One of `idle`, `visibility_only`, `selective`, `full`. |
...
| `microsegment.io/enforcement` | `idle`, `visibility_only`, `selective`, `full` | Override the enforcement mode for this namespace. |
